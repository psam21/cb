...
1. **Sidebar CTA:** What action should heritage detail pages promote?
   - Option A: "Share this contribution"
   - Option B: "Contribute similar"
   - Option C: Leave as-is (metadata only)

2. **Prominent Display:** What heritage info is most important to highlight?
   - Option A: Heritage Type + Time Period
   - Option B: Category + Region
   - Option C: Leave as-is (no prominence)

3. **Type Pattern:** Generic or specific type aliases?
   - Option A: All use generic `ContentDetail<T>`
   - Option B: All use specific aliases (`ShopContentDetail`, `HeritageContentDetail`)
   - Option C: Keep as-is (mixed)

4. **Category Resolution:** Does heritage use category IDs that need resolution?
   - If yes: Add `getCategoryById()` support
   - If no: Categories are already human-readable

---

*Document created: October 1, 2025*  
*Status: Awaiting review and prioritization*  
*Related: heritage-architectural-issues-and-fixes.md*
