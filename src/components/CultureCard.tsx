  ...
  return (
    <div className={`culture-card ${featured ? 'lg:col-span-2' : ''}`}>
      <div className="relative aspect-video overflow-hidden">
        <Image
          src={culture.image}
          alt={`${culture.name} cultural scene`}
          fill
      ...
      <div className="p-6">
        <h3 className="text-xl font-serif font-bold text-primary-800 mb-2">{culture.name}</h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {culture.tags.slice(0, featured ? 4 : 2).map((tag) => (
            <span key={tag} className="px-2 py-1 bg-primary-50 text-primary-700 rounded-md text-xs">
              {tag}
            </span>
          ))}
