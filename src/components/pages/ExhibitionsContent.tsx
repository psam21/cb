...
function filterExhibitions(
  ...
  return items.filter((ex) => {
    const matchesTerm =
      ...
      ex.culture.toLowerCase().includes(term) ||
...
export default function ExhibitionsContent() {
  const { exhibitions, categories, regions } = exhibitionsData;
  ...
  const regular = sorted.filter((e) => !e.featured);
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="flex flex-wrap items-center text-sm text-gray-600 mb-3 gap-x-4 gap-y-1">
                      <span className="font-medium text-accent-600">{ex.category}</span>
                      <span className="flex items-center">
                        <MapPin className="w-4 h-4 mr-1" />
                        {ex.region}
                      </span>
                      <span>{ex.contributors} contributors</span>
                    </div>
